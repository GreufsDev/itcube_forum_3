...
def admin_panel():
    ...
    # Получаем список тем для вкладки "Темы"
    conn = get_db_connection()
    cursor = conn.cursor()
    topics = cursor.execute('''
        ...
        CASE 
            ...
            WHEN t.status = 'approved' THEN 'success'
...
@app.route('/api/topics/<int:topic_id>/moderate', methods=['POST'])
@login_required
@admin_required
def moderate_topic_api(topic_id):
    try:
        data = request.get_json()
        ...
        if status not in ['approved', 'rejected']:
            return jsonify({
                'success': False,
                'message': 'Неверный статус модерации'
            }), 400
        ...
        topic = cursor.execute('SELECT * FROM topics WHERE id = ?', (topic_id,)).fetchone()
        if not topic:
            conn.close()
            return jsonify({
                'success': False,
        ...
        cursor.execute('''
            ...
            WHERE id = ?
        ''', (status, session['user_id'], topic_id))
        ...
        log_event(
            session['user_id'],
            'moderate_topic',
            f'Тема "{topic["title"]}" {status}',
            'success'
        )

        return jsonify({
            'success': True,
            'message': f'Тема успешно {"одобрена" if status == "approved" else "отклонена"}'
        })
        ...
        return jsonify({
            'success': False,
            'message': 'Произошла ошибка при модерации темы'
        }), 500
